...
--Return what medal a competitor won instead of what place a player took 
SELECT ID, 
	CASE
    	WHEN PLACE LIKE '%FIRST%' THEN REPLACE(PLACE, 'FIRST', 'GOLD MEDAL')
    	WHEN PLACE LIKE '%SECOND%' THEN REPLACE(PLACE, 'SECOND', 'SILVER MEDAL')
    	WHEN PLACE LIKE '%THIRD%' THEN REPLACE(PLACE, 'THIRD', 'BRONZE MEDAL')
    	ELSE PLACE
END AS TYPE_OF_MEDAL,
NAME_1
FROM RESULTS
